...
bool ProjectionTwoFrameTwoCamFactor::Evaluate(double const *const *parameters, double *residuals,
  ...
  if (jacobians) {
    ...
    if (jacobians[2]) {
      ...
      jaco_ex.rightCols<3>() =
          ric2.transpose() * Rj.transpose() * Ri * ric * -Utility::skewSymmetric(pts_camera_i);
...
void ProjectionTwoFrameTwoCamFactor::check(double **parameters) {
  ...
  std::cout << Eigen::Map<Eigen::Matrix<double, 2, 7, Eigen::RowMajor>>(jaco[3]) << std::endl << std::endl;
