...
int main(int argc, char **argv) {
  ...
  if (outFile == NULL) printf("Output path dosen't exist: %s\n", OUTPUT_FOLDER.c_str());
  ...
  for (size_t i = 0; i < imageTimeList.size(); i++) {
    if (ros::ok()) {
      ...
      //	    pos_accuracy, vel_accuracy, navstat, numsats, velmode,
      // orimode);
      ...
      if (outFile != NULL)
        fprintf(outFile, "%f %f %f %f %f %f %f %f %f %f %f %f \n", pose(0, 0), pose(0, 1), pose(0, 2),
                pose(0, 3), pose(1, 0), pose(1, 1), pose(1, 2), pose(1, 3), pose(2, 0), pose(2, 1),
                pose(2, 2), pose(2, 3));
